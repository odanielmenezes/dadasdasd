...
const PhotoViwer = () => {
    ...
    const getData = useCallback(async (url, nomeHover, pagina, sol, camera) => {
        const response = await fetch(`${url}/mars-photos/api/v1/rovers/${nomeHover}/photos?sol=${sol}${camera === "todas" ? "" : `&camera=${camera}`}&page=${pagina}&api_key=R8U8ohxv45wym3DvNxTFrASF1jyH6deia6heghSc`, {
            method: "GET"
        })
            .then(res => res.json())
        setData(response)
